# CodexOps Agent Guide

All docs must canonical, no past commentary, only live state.

## Project Summary
CodexOps is a Tauri app that orchestrates Codex agents across local workspaces.

- Frontend: React + Vite
- Backend (app): Tauri Rust process
- Backend (daemon): `src-tauri/src/bin/codex_ops_daemon.rs`
...
- Daemon wiring and transport concerns: `src-tauri/src/bin/codex_ops_daemon.rs`
...
The daemon defines wrapper modules named `codex` and `files` inside `src-tauri/src/bin/codex_ops_daemon.rs`.
...
- Daemon entrypoint: `src-tauri/src/bin/codex_ops_daemon.rs`
...
3. Daemon-only transport/wiring behavior:
   - Update `src-tauri/src/bin/codex_ops_daemon.rs`.
...
3. Wire it in the daemon.
   ...
   - Add the JSON-RPC handler branch in `codex_ops_daemon.rs`.
...
- Daemon spawn adapter:
  - `spawn_with_client(...)` in `src-tauri/src/bin/codex_ops_daemon.rs`
- Daemon wrapper modules:
  - `mod codex { ... }` and `mod files { ... }` in `codex_ops_daemon.rs`
...
- Workspaces/worktrees:
  ...
  - Daemon wiring: `src-tauri/src/bin/codex_ops_daemon.rs`
