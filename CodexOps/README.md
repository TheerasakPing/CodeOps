...
![CodexOps](screenshot.png)

CodexOps is a Tauri app for orchestrating multiple Codex agents across local workspaces. It provides a sidebar to manage projects, a home screen for quick actions, and a conversation view backed by the Codex app-server protocol.
...
2. On desktop CodexOps, open `Settings > Server`.
...
7. On iOS CodexOps, open `Settings > Server`.
...
src-tauri/
  ...
  src/bin/codex_ops_daemon.rs  remote daemon JSON-RPC process
...
- The remote daemon entrypoint is `src-tauri/src/bin/codex_ops_daemon.rs`; shared domain logic lives in `src-tauri/src/shared/`.
