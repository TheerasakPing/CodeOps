# CodexOps iOS Remote Blueprint (Orbit + Tailscale Bootstrap)

This document is the canonical implementation plan for shipping CodexOps on iOS with a Tailscale-first bootstrap path and Orbit on Cloudflare as the production relay/control plane to a macOS runner.
...
- Make macOS setup manageable from CodexOps Settings: authenticate, pair device, launch/stop runner, inspect status/logs.
...
- No hosted CodexOps service required.
...
## Required Backend Refactor in CodexOps
...
- Basic runner lifecycle controls are implemented via Tauri commands in `src-tauri/src/orbit/mod.rs` and daemon Orbit mode args in `src-tauri/src/bin/codex_ops_daemon.rs`.
...
Extend daemon binary (`src-tauri/src/bin/codex_ops_daemon.rs`) with optional Orbit connector mode.
...
2. In CodexOps Settings, set `Backend Mode = Remote`, `Provider = TCP`.
