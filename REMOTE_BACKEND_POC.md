...
This fork includes a **proof-of-concept** daemon that runs CodexOps's backend logic in a separate process (intended for WSL2/Linux), exposing a simple **line-delimited JSON-RPC** protocol over TCP.
...
cargo run --bin codex_ops_daemon -- \
  --listen 127.0.0.1:4732 \
  --data-dir "$HOME/.local/share/codex-ops-daemon" \
  --token "$TOKEN"
```
