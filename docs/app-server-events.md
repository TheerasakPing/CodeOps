...
- Which app-server events CodexOps supports right now.
- Which app-server requests CodexOps sends right now.
- Where to look in CodexOps to add support.
...
2. Compare Codex events vs CodexOps routing.
3. Compare Codex client request methods vs CodexOps outgoing request methods.
4. Compare Codex server request methods vs CodexOps inbound request handling.
5. Update supported and missing lists below.

## Where To Look In CodexOps
...
- `src-tauri/src/bin/codex_ops_daemon.rs`
...
## Supported Requests (CodexOps -> App-Server, v2)

These are v2 request methods CodexOps currently sends to Codex app-server:
...
Compared against Codex v2 request methods, CodexOps currently does not send:
...
## Server Requests (App-Server -> CodexOps, v2)
...
3. List CodexOps routed methods:
...
4. List CodexOps outgoing requests:
...
6. Check for camelCase vs snake_case mismatches:
   ...
   - CodexOps generally defends against this by checking both forms (for example in `threadNormalize.ts` and `useAppServerEvents.ts`), while centralizing method/type parsing in `appServerEvents.ts`.
...
- Steering behavior while a turn is processing:
  - CodexOps attempts `turn/steer` when steering is enabled and an active turn exists.
  - If the server/daemon reports unknown `turn/steer`/`turn_steer`, CodexOps
