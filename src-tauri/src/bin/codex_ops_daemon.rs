...
#[path = "../storage.rs"]
mod storage;
#[path = "codex_ops_daemon/transport.rs"]
mod transport;
#[allow(dead_code)]
#[path = "../types.rs"]
mod types;
...
const DAEMON_NAME: &str = "codex-ops-daemon";
...
fn default_data_dir() -> PathBuf {
    if let Ok(xdg) = env::var("XDG_DATA_HOME") {
        ...
        if !trimmed.is_empty() {
            return PathBuf::from(trimmed).join("codex-ops-daemon");
        }
    }
    ...
    PathBuf::from(home)
        .join(".local")
        .join("share")
        .join("codex-ops-daemon")
}

fn usage() -> String {
    format!(
        "\
USAGE:\n  codex-ops-daemon [--listen <addr>] [--data-dir <path>] [--token <token> | --insecure-no-auth]\n  codex-ops-daemon --orbit-url <ws-url> [--orbit-token <token>] [--orbit-auth-url <url>] [--orbit-runner-name <name>] [--data-dir <path>]\n\n\
...
mod tests {
    ...
    fn test_state(data_dir: &std::path::Path) -> DaemonState {
        ...
        DaemonState {
            ...
            daemon_binary_path: Some("/tmp/codex-ops-daemon".to_string()),
        }
    }
...
fn main() {
    ...
    runtime.block_on(async move {
        ...
        if config.orbit_url.is_some() {
            eprintln!(
                "codex-ops-daemon orbit mode (data dir: {})",
                state
                    .storage_path
                    .parent()
        ...
        eprintln!(
            "codex-ops-daemon listening on {} (data dir: {})",
            config.listen,
            state
                .storage_path
                .parent()
