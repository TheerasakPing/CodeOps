...
export function useTerminalTabs({
  ...
  const createTerminal = useCallback((workspaceId: string, initialInput?: string) => {
    ...
    setTabsByWorkspace((prev) => {
      ...
      return {
        ...prev,
        [workspaceId]: [...existing, { id, title, initialInput }],
      };
    });
  ...
  const ensureTerminalWithTitle = useCallback(
    (workspaceId: string, terminalId: string, title: string, initialInput?: string) => {
      setTabsByWorkspace((prev) => {
        const existing = prev[workspaceId] ?? [];
        ...
        if (index === -1) {
          return {
            ...prev,
            [workspaceId]: [...existing, { id: terminalId, title, initialInput }],
          };
        }
